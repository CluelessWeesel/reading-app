...
import { rangeStartFor } from "./logDateRange";
import type { CurrentBookForLog } from "./types";

// The per-book entry form for a single target date -- shared by the
// Backfill tab (own date picker) and Tonight tab (arrow-stepped to a past
// day), so gap detection/spreading and the save behave identically either
// way you get here.
export function BackfillForm({
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch("/api/log/backfill", {
        ...
        body: JSON.stringify({ date: targetDate, today: todayLocalIso(), entries: loggableEntries }),
      });
      ...
      if (successCount > 0) setSavedMessage(`Logged ${successCount} book${successCount === 1 ? "" : "s"} for ${targetDate}.`);
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
        ...
        return (
          <div key={book.book_id} className="rounded-xl border border-gold bg-surface-1 p-4">
            ...
            {beforeRange ? (
              ...
              <>
                {gapDays > 1 && (
                  <div className="mb-2 rounded-lg bg-hover px-3 py-2 text-sm text-ink-warm-faint">
                      ...
                      <button
                        ...
                        className={`flex-1 rounded-full px-2 py-1 text-xs transition ${
                          (spreadChoice[book.book_id] ?? "even") === "even"
                            ...
                            : "text-ink-warm-muted hover:text-ink-warm"
                        }`}
                      >
                        Spread evenly
                      </button>
                ...
                <label className="mb-1 block text-sm font-medium text-ink-warm" htmlFor={`backfill-pos-${book.book_id}`}>
      ...
      {savedMessage && <p className="text-center text-sm text-ink-warm-faint">{savedMessage}</p>}
    </div>
  );
}
