...
// Shared by /stats and /stats/compare -- same queries, same shapes, so a
// second scope-comparison view over this data needs zero new SQL.
...
export async function getStatsData(): Promise<StatsData> {
  ...
  const [dailyRows, formatDailyRows, bookRows, goals, seriesParents, percentiles, allGenres, docCounts, tbrEntries, appSettings] =
    await Promise.all([
      getDailyRows(today),
  ...
  return { dailyRows, formatDailyRows, books, goals, seriesParents, allGenres, tbrEntries, appSettings, today, currentYear, years: YEARS };
}
