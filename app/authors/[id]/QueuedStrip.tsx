...
export function QueuedStrip({ entries }: { entries: QueuedEntry[] }) {
  return (
    <div>
      <h2 className={`${fraunces.className} mb-3 text-lg font-semibold text-ink`}>
        Queued <span className="text-sm font-normal text-ink-faint">({entries.length})</span>
      </h2>
      <div className="flex gap-3 overflow-x-auto pb-1">
        {entries.map((entry) => (
          <div key={entry.id} className="flex w-20 shrink-0 flex-col gap-1">
            ...
            <p className="truncate text-xs text-ink-faint">{entry.title}</p>
          </div>
        ))}
      </div>
    </div>
